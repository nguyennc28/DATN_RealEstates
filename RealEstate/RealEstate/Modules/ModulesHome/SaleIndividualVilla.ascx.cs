...
{
    public partial class SaleIndividualVilla : System.Web.UI.UserControl
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                LoadSaleIndividualVilla();
            }
        }
        private void LoadSaleIndividualVilla()
        {
            string chuoi = "";
            List<HomeInfo> listApartmentInfos = new List<HomeInfo>();
            listApartmentInfos = HomeService.HomeInfo_GetByTop("10", "HomeTypeID = 3", "");
